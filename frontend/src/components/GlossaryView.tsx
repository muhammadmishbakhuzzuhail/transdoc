...
export function GlossaryView() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader><CardTitle>{t("tab_glossary")} — {entries.length} {t("gl_terms_n")}</CardTitle></CardHeader>
        <CardContent className="space-y-1">
          {entries.length === 0 && (
            <p className="py-6 text-center text-sm text-muted-foreground">{t("gl_empty")}</p>
          )}
          {entries.map((e) => (
      ...
      {sugg.length > 0 && (
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0">
            <CardTitle>{t("gl_suggestions")} ({sugg.length})</CardTitle>
            <Button size="sm" variant="secondary" disabled={!picked.size}
              ...
              <Check className="h-3.5 w-3.5" /> {t("gl_accept")} ({picked.size})
            </Button>
          </CardHeader>
          <CardContent className="space-y-1">
            {sugg.map((s) => {
    ...
    <Card>
      <CardContent className="flex flex-wrap items-center gap-2 pt-5 text-sm">
        <a href={glossaryCsvUrl()}>
          <Button size="sm" variant="outline"><Download className="h-4 w-4" /> {t("gl_export_csv")}</Button>
        </a>
        ...
        <Button size="sm" variant="outline" onClick={() => csvRef.current?.click()}>
          <Upload className="h-4 w-4" /> {t("gl_import_csv")}
        </Button>
        ...
        <a href={tmTmxUrl()}>
          <Button size="sm" variant="outline"><Download className="h-4 w-4" /> {t("gl_export_tmx")}</Button>
        </a>
        <Button size="sm" variant="outline" onClick={() => tmxRef.current?.click()}>
          <Upload className="h-4 w-4" /> {t("gl_import_tmx")}
        </Button>
  ...
  return (
    <Card>
      <CardContent className="flex flex-wrap items-end gap-2 pt-5">
        <input className={`${inp} w-40`} placeholder={t("gl_ph_term")} value={f.term}
          onChange={(e) => set("term", e.target.value)} />
        <input className={`${inp} w-40`} placeholder={t("gl_ph_rendering")} value={f.rendering}
        ...
        <input className={`${inp} w-28`} placeholder={t("gl_ph_domain")} value={f.domain}
        ...
        <Button size="sm" disabled={!f.term.trim() || !f.rendering.trim()}
          ...
          <Plus className="h-4 w-4" /> {t("gl_add")}
        </Button>
      </CardContent>
    </Card>
  )
